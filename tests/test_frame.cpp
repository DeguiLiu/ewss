...
TEST_CASE("Frame parse - text frame unmasked", "[frame]") {
  ...
  size_t consumed = ws::parse_frame_header(
...
TEST_CASE("Frame parse - 65536-byte payload (64-bit length)", "[frame]") {
  ...
  ws::FrameHeader header;
  size_t consumed = ws::parse_frame_header(
      std::string_view(reinterpret_cast<const char*>(frame), 10),
      header);
  REQUIRE(consumed == 10);
...
TEST_CASE("Frame parse - incomplete header (1 byte)", "[frame]") {
  ...
  ws::FrameHeader header;
  size_t consumed = ws::parse_frame_header(
      std::string_view(reinterpret_cast<const char*>(frame), 1),
      header);
  REQUIRE(consumed == 0);
}
...
TEST_CASE("Frame parse - incomplete extended length", "[frame]") {
  ...
  ws::FrameHeader header;
  size_t consumed = ws::parse_frame_header(
      std::string_view(reinterpret_cast<const char*>(frame), 3),
      header);
  REQUIRE(consumed == 0);
}
...
TEST_CASE("Frame parse - incomplete mask key", "[frame]") {
  ...
  ws::FrameHeader header;
  size_t consumed = ws::parse_frame_header(
      std::string_view(reinterpret_cast<const char*>(frame), 4),
      header);
  REQUIRE(consumed == 0);
}
